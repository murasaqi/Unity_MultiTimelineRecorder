...
namespace BatchRenderingTool.TestAutomation
{
    ...
    public static class MCPTestIntegration
    {
        ...
        {
            Debug.Log("[MCPTestIntegration] Unity Test RunnerのExport Resultボタンを自動実行します");
        ...
        {
            Debug.Log("[MCPTestIntegration] テストを実行してからExport Resultボタンを自動実行します");
        ...
        {
            Debug.Log("[MCPTestIntegration] MCPコマンド経由でXMLエクスポート（Native版）を実行します");
        ...
        {
            Debug.Log("[MCPTestIntegration] MCPコマンド経由でXMLエクスポート（カスタム版）を実行します");
        ...
        {
            Debug.Log("[MCPTestIntegration] 完全なテスト実行とネイティブエクスポートを開始します");
            ...
            EditorApplication.delayCall += () =>
            {
                EditorApplication.delayCall += () =>
                {
                    Debug.Log("[MCPTestIntegration] テスト実行が完了しました。ネイティブXMLエクスポートを実行します");
                    TestRunnerExportAutomation.AutoExportTestResults();
                };
            };
        }
        ...
        {
            Debug.Log("[MCPTestIntegration] エクスポート方法の比較検証を開始します");
            ...
            Debug.Log($"[MCPTestIntegration] カスタムエクスポート完了: {customPath}");
            
            // 2. ネイティブエクスポート実行（ユーザーが保存先を選択）
            EditorApplication.delayCall += () =>
            {
                Debug.Log("[MCPTestIntegration] ネイティブエクスポートを実行します（保存ダイアログが表示されます）");
                TestRunnerExportAutomation.AutoExportTestResults();
                
                Debug.Log("[MCPTestIntegration] 両方のXMLファイルを比較して、エクスポート方法の違いを確認してください");
            };
        }
