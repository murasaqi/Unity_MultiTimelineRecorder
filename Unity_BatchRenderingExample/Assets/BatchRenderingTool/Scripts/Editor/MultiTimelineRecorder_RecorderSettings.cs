...
namespace BatchRenderingTool
{
    ...
    public partial class MultiTimelineRecorder
    {
        ...
        private List<RecorderSettings> CreateAOVRecorderSettings(string outputPath, string outputFileName)
        {
            ...
            {
                BatchRenderingToolLogger.LogError($"[MultiTimelineRecorder] Invalid AOV configuration: {errorMessage}");
                return null;
            }
        ...
        {
            BatchRenderingToolLogger.Log($"[MultiTimelineRecorder] === CreateAlembicRecorderSettings called with path: {outputPath}, fileName: {outputFileName} ===");
            ...
            if (!config.Validate(out errorMessage))
            {
                BatchRenderingToolLogger.LogError($"[MultiTimelineRecorder] Invalid Alembic configuration: {errorMessage}");
                return null;
            }
        ...
        private RecorderSettings CreateAnimationRecorderSettings(string outputPath, string outputFileName)
        {
            ...
            if (!config.Validate(out errorMessage))
            {
                BatchRenderingToolLogger.LogError($"[MultiTimelineRecorder] Invalid Animation configuration: {errorMessage}");
                return null;
            }
        ...
        {
            BatchRenderingToolLogger.Log($"[MultiTimelineRecorder] === CreateFBXRecorderSettings called with path: {outputPath}, fileName: {outputFileName} ===");
            
            // FBX recorder is not supported in single recorder mode for multi-timeline
            BatchRenderingToolLogger.LogError("[MultiTimelineRecorder] FBX Recorder is not supported in single recorder mode. Use per-timeline configuration.");
            return null;
        }
        ...
        private RecorderSettings CreateMovieRecorderSettingsFromConfig(string outputPath, string outputFileName, MultiRecorderConfig.RecorderConfigItem config)
        {
            ...
            {
                BatchRenderingToolLogger.LogError($"[MultiTimelineRecorder] Invalid movie configuration: {errorMessage}");
                return null;
            }
        ...
        private RecorderSettings CreateFBXRecorderSettingsFromConfig(string outputPath, string outputFileName, MultiRecorderConfig.RecorderConfigItem config)
        {
            ...
            {
                BatchRenderingToolLogger.LogError($"[MultiTimelineRecorder] FBX Recorder config is null for recorder '{config.name}'.");
                return null;
            }
            
            if (config.fbxConfig.targetGameObject == null)
            {
                BatchRenderingToolLogger.LogError($"[MultiTimelineRecorder] FBX Recorder requires a target GameObject to be set for recorder '{config.name}'.");
                return null;
            }
            ...
            if (!settingsConfig.Validate(out errorMessage))
            {
                BatchRenderingToolLogger.LogError($"[MultiTimelineRecorder] Invalid FBX configuration: {errorMessage}");
                return null;
            }
        ...
        private RecorderSettings CreateRecorderSettingsForItem(MultiRecorderConfig.RecorderConfigItem recorderItem, PlayableDirector director, int timelineIndex)
        {
            try
            {
                BatchRenderingToolLogger.LogVerbose($"[MultiTimelineRecorder] Creating RecorderSettings for {recorderItem.name} on timeline {director.gameObject.name}");
                ...
                BatchRenderingToolLogger.LogVerbose($"[MultiTimelineRecorder] Output path: {processedFilePath}, Filename: {processedFileName}");
            ...
            switch (recorderItem.recorderType)
            {
                ...
                case RecorderSettingsType.AOV:
                    ...
                    if (aovSettingsList != null && aovSettingsList.Count > 0)
                    {
                        ...
                        {
                            BatchRenderingToolLogger.LogWarning($"[MultiTimelineRecorder] Multiple AOV outputs detected ({aovSettingsList.Count}), only using the first one for timeline {director.gameObject.name}");
                            BatchRenderingToolLogger.LogWarning($"[MultiTimelineRecorder] Consider selecting only one AOV type to avoid this limitation");
                        }
                        
                        BatchRenderingToolLogger.LogVerbose($"[MultiTimelineRecorder] Using AOV recorder settings: {recorderSettings.name}");
                    }
                    else
                    {
                        BatchRenderingToolLogger.LogError($"[MultiTimelineRecorder] Failed to create AOV recorder settings for timeline {director.gameObject.name}");
                    }
                    break;
                ...
                default:
                    BatchRenderingToolLogger.LogError($"[MultiTimelineRecorder] Unsupported recorder type: {recorderItem.recorderType}");
                    break;
            }
            ...
            {
                BatchRenderingToolLogger.LogError($"[MultiTimelineRecorder] Exception creating RecorderSettings for {recorderItem.name}: {e.Message}");
