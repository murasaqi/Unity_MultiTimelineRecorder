...
namespace Unity.MultiTimelineRecorder
{
    ...
    public class AlembicRecorderSettingsConfig
    {
        ...
        {
            MultiTimelineRecorderLogger.LogVerbose($"[AlembicRecorderSettingsConfig] Creating Alembic settings: {name}");
            ...
            if (alembicRecorderSettingsType == null)
            {
                MultiTimelineRecorderLogger.LogError("[AlembicRecorderSettingsConfig] AlembicRecorderSettings type not found. Make sure Unity Recorder and Alembic packages are installed.");
                return null;
            }
            ...
            if (settings == null)
            {
                MultiTimelineRecorderLogger.LogError($"[AlembicRecorderSettingsConfig] Failed to create instance of {alembicRecorderSettingsType.Name}");
                return null;
            }
            ...
            MultiTimelineRecorderLogger.LogVerbose($"[AlembicRecorderSettingsConfig] Export targets: {exportTargets}");
            MultiTimelineRecorderLogger.LogVerbose($"[AlembicRecorderSettingsConfig] Frame rate: {frameRate}, Samples per frame: {samplesPerFrame}");
            MultiTimelineRecorderLogger.LogVerbose($"[AlembicRecorderSettingsConfig] Scale: {scaleFactor}, Handedness: {handedness}");
        ...
        private void ApplyAlembicSettings(RecorderSettings settings)
        {
            ...
            MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] === Applying settings to type: {settingsType.FullName} ===");
            MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] === Export scope: {exportScope}, Target GameObject: {targetGameObject?.name ?? "null"} ===");
            ...
            if (settingsProperty != null && settingsProperty.CanRead)
            {
                ...
                {
                    MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] Found inner Settings of type: {innerSettings.GetType().FullName}");
                    ...
                    if (innerScopeProperty != null && innerScopeProperty.CanWrite)
                    {
                        ...
                        MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] Found Scope property of type: {exportScopeType.FullName}");
                        ...
                        if (unityScope != null)
                        {
                            ...
                            MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] === Successfully set Scope to {unityScope} ===");
                        }
                    }
                        ...
                        targetGameObject != null)
                    {
                        ...
                        if (targetBranchProperty != null && targetBranchProperty.CanWrite)
                        {
                            ...
                            MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] === Successfully set TargetBranch to {targetGameObject.name} ===");
                        }
                    }
            ...
            {
                MultiTimelineRecorderLogger.LogVerbose($"[AlembicRecorderSettingsConfig] Field: {field.Name} (Type: {field.FieldType.Name})");
                    ...
                    field.Name.ToLower().Contains("exportscope"))
                {
                    ...
                    if (field.FieldType.IsEnum)
                    {
                        try
                        {
                            ...
                            foreach (var enumValue in enumValues)
                            {
                                    ...
                                    (enumName.Contains("target") || enumName.Contains("branch") || enumName.Contains("gameobject")))
                                {
                                    ...
                                    scopeSet = true;
                                    MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] Set field {field.Name} to {enumValue}");
                                    break;
                                }
                        ...
                        {
                            MultiTimelineRecorderLogger.LogWarning($"[AlembicRecorderSettingsConfig] Failed to set field {field.Name}: {e.Message}");
                        }
                    }
                }
                         ...
                         field.Name.ToLower().Contains("branch"))
                {
                    ...
                    if (field.FieldType == typeof(GameObject) && targetGameObject != null)
                    {
                        ...
                        MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] Set field {field.Name} to {targetGameObject.name}");
                    }
                }
            }
            
            // This code path should not be reached if inner Settings were found
            MultiTimelineRecorderLogger.LogError("[AlembicRecorderSettingsConfig] Failed to access inner Settings object");
            ...
            if (scopeProperty != null && scopeProperty.PropertyType.IsEnum)
            {
                ...
                {
                    string enumName = enumValue.ToString().ToLower();
                    MultiTimelineRecorderLogger.LogVerbose($"[AlembicRecorderSettingsConfig] Found Scope enum value: {enumValue} ({enumName})");
            ...
            {
                scopeSet = true;
                MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] === Successfully set Scope to {scopeValue} (matched enum) ===");
            }
            else
            {
                ...
                {
                    scopeSet = true;
                    MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] === Successfully set Scope to {scopeInt} (int) ===");
                }
                else
                {
                    ...
                    {
                        scopeSet = true;
                        MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] === Successfully set Scope to {scopeInt} (int reversed) ===");
                    }
                }
            }
            ...
            if (!scopeSet)
            {
                MultiTimelineRecorderLogger.LogError($"[AlembicRecorderSettingsConfig] === FAILED to set scope property ===");
                
                // Log all fields for debugging
                MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] === Available fields on {settingsType.Name}: ===");
                var allFields = settingsType.GetFields(BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Public);
                foreach (var field in allFields)
                {
                    MultiTimelineRecorderLogger.Log($"  - Field: {field.Name} (Type: {field.FieldType.Name})");
                }
            }
            ...
            {
                MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] === Setting TargetBranch to: {targetGameObject.name} ===");
                ...
                {
                    success = true;
                    MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] === Successfully set 'TargetBranch' property ===");
                }
                ...
                else
                {
                    ...
                    {
                        success = true;
                        MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] === Successfully set 'TargetGameObject' property ===");
                    }
                    else if (SetPropertyValue(settings, settingsType, "GameObject", targetGameObject))
                    {
                        success = true;
                        MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] === Successfully set 'GameObject' property ===");
                    }
                }
                
                if (!success)
                {
                    MultiTimelineRecorderLogger.LogError($"[AlembicRecorderSettingsConfig] === FAILED to set target GameObject on AlembicRecorderSettings ===");
                    LogAvailableProperties(settingsType);
                }
                else
                {
        ...
        private bool SetPropertyValue(object obj, System.Type type, string propertyName, object value)
        {
            try
            {
                ...
                if (property != null && property.CanWrite)
                {
                    ...
                    MultiTimelineRecorderLogger.LogVerbose($"[AlembicRecorderSettingsConfig] Found property {propertyName} of type {property.PropertyType.Name}");
                    ...
                    if (value != null && property.PropertyType.IsEnum && value.GetType() != property.PropertyType)
                    {
                        ...
                        try
                        {
                            ...
                            MultiTimelineRecorderLogger.LogVerbose($"[AlembicRecorderSettingsConfig] Converted value from {value.GetType().Name} to {property.PropertyType.Name}");
                        }
                        catch (System.Exception convEx)
                        {
                            MultiTimelineRecorderLogger.LogWarning($"[AlembicRecorderSettingsConfig] Failed to convert value: {convEx.Message}");
                            return false;
                        }
                    }
                    ...
                    MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] === Set property {propertyName} to {convertedValue}, verified: {verifyValue} ===");
                    return true;
                }
                else
                {
                    ...
                    if (field != null)
                    {
                        ...
                        MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] === Set field {propertyName} to {value}, verified: {verifyValue} ===");
                        return true;
                    }
                }
                
                // Property/field not found
                MultiTimelineRecorderLogger.LogVerbose($"[AlembicRecorderSettingsConfig] Property/field {propertyName} not found on type {type.Name}");
                return false;
            }
            catch (System.Exception e)
            {
                MultiTimelineRecorderLogger.LogWarning($"[AlembicRecorderSettingsConfig] Failed to set {propertyName}: {e.Message}");
                return false;
            }
        }
        ...
        {
            MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] === Available properties on {type.Name}: ===");
            
            var properties = type.GetProperties();
            foreach (var prop in properties)
            {
                MultiTimelineRecorderLogger.Log($"  - Property: {prop.Name} (Type: {prop.PropertyType.Name}, CanWrite: {prop.CanWrite})");
            }
        ...
        private void VerifyTargetGameObjectSetting(RecorderSettings settings, System.Type settingsType)
        {
            ...
            foreach (var propName in properties)
            {
                ...
                if (prop != null && prop.CanRead)
                {
                    ...
                    if (value != null)
                    {
                        MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] === VERIFIED: {propName} = {value} ===");
                        return;
                    }
                }
            }
            ...
            if (scopeProp != null && scopeProp.CanRead)
            {
                ...
                MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] === VERIFIED: Scope = {scopeValue} ===");
            }
        }
        ...
        private void SetupAlembicInputSettings(RecorderSettings settings)
        {
            try
            {
                ...
                foreach (var typeName in possibleTypeNames)
                {
                    ...
                    {
                        MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] Found AlembicInputSettings type: {typeName}");
                        break;
                    }
                }
                ...
                if (alembicInputSettingsType == null)
                {
                    ...
                    if (inputSettingsProperty != null)
                    {
                        ...
                        if (defaultInputSettings != null)
                        {
                            ...
                            MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] Found InputSettings type from settings: {alembicInputSettingsType.FullName}");
                        }
                    }
                }
                ...
                if (alembicInputSettingsType != null)
                {
                    ...
                    {
                        MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] Created AlembicInputSettings instance");
                        ...
                        if (inputSettingsProperty != null && inputSettingsProperty.CanWrite)
                        {
                            ...
                            MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] Set InputSettings on recorder settings");
                        }
                        else
                        {
                            ...
                            if (setInputMethod != null)
                            {
                                ...
                                MultiTimelineRecorderLogger.Log($"[AlembicRecorderSettingsConfig] Set InputSettings via method");
                            }
                        }
                    }
                }
            ...
            {
                MultiTimelineRecorderLogger.LogWarning($"[AlembicRecorderSettingsConfig] Failed to setup AlembicInputSettings: {e.Message}");
            }
        }
