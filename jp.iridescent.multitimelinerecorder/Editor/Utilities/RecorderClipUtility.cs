...
namespace Unity.MultiTimelineRecorder
{
    ...
    public static class RecorderClipUtility
    {
        ...
        public static void EnsureRecorderTypeIsSet(RecorderClip clip, RecorderSettings settings)
        {
            ...
            {
                MultiTimelineRecorderLogger.LogWarning($"[RecorderClipUtility] Failed to set recorder type via reflection: {e.Message}");
            }
        }
        ...
        public static ImageRecorderSettings CreateProperImageRecorderSettings(string name)
        {
            ...
            if (inventoryType != null)
            {
                ...
                if (createMethod != null)
                {
                    try
                    {
                        ...
                        {
                            settings.name = name;
                            MultiTimelineRecorderLogger.LogVerbose("[RecorderClipUtility] Created ImageRecorderSettings using RecordersInventory");
                            return settings;
                        }
                    }
                    catch (System.Exception e)
                    {
                        MultiTimelineRecorderLogger.LogWarning($"[RecorderClipUtility] Failed to use RecordersInventory: {e.Message}");
                    }
                }
            }
            ...
            MultiTimelineRecorderLogger.LogVerbose($"[RecorderClipUtility] Created ImageRecorderSettings directly: {imageSettings.GetType().FullName}");
        ...
        public static RecorderSettings CreateProperAlembicRecorderSettings(string name)
        {
                ...
                if (createMethod != null)
                {
                    try
                    {
                        ...
                        if (alembicType != null)
                        {
                            ...
                            {
                                settings.name = name;
                                MultiTimelineRecorderLogger.LogVerbose("[RecorderClipUtility] Created AlembicRecorderSettings using RecordersInventory");
                                return settings;
                            }
                        }
                    }
                    ...
                    {
                        MultiTimelineRecorderLogger.LogWarning($"[RecorderClipUtility] Failed to use RecordersInventory for Alembic: {e.Message}");
                    }
                }
            }
            ...
            if (alembicRecorderType != null)
            {
                ...
                if (alembicSettings != null)
                {
                    ...
                    MultiTimelineRecorderLogger.LogVerbose($"[RecorderClipUtility] Created AlembicRecorderSettings directly: {alembicSettings.GetType().FullName}");
                    return alembicSettings;
                }
            }
            
            MultiTimelineRecorderLogger.LogError("[RecorderClipUtility] Failed to create AlembicRecorderSettings. Make sure Alembic package is installed.");
            return null;
        }
        ...
        private static void ApplyAlembicSpecificSettings(RecorderClip clip, RecorderSettings settings)
        {
            ...
            {
                MultiTimelineRecorderLogger.LogWarning($"[RecorderClipUtility] Failed to apply Alembic-specific settings: {e.Message}");
            }
        }
        ...
        {
            MultiTimelineRecorderLogger.LogVerbose($"[RecorderClipUtility] Creating recorder settings for type: {typeName}");
            ...
            if (inventoryType != null)
            {
                ...
                if (createMethod != null)
                {
                    try
                    {
                        MultiTimelineRecorderLogger.Log($"[RecorderClipUtility] Using RecordersInventory to create: {typeName}");
                        ...
                        switch (typeName.ToLower())
                        {
                            ...
                            case "fbxrecorder":
                                MultiTimelineRecorderLogger.Log("[RecorderClipUtility] Looking for FBX recorder type...");
                                    ...
                                    if (assembly.FullName.Contains("Fbx"))
                                    {
                                        try
                                        {
                                            ...
                                            foreach (var type in types)
                                            {
                                                ...
                                                {
                                                    settingsType = type;
                                                    MultiTimelineRecorderLogger.Log($"[RecorderClipUtility] Found FBX type: {type.FullName}");
                                                    break;
                                                }
                                            }
                                        }
                                ...
                                {
                                    MultiTimelineRecorderLogger.LogError("[RecorderClipUtility] FbxRecorderSettings not found. Install Unity FBX Exporter package.");
                                    return null;
                                }
                                break;
                        ...
                        if (settingsType != null)
                        {
                            MultiTimelineRecorderLogger.Log($"[RecorderClipUtility] About to create {settingsType.FullName} using RecordersInventory");
                            var settings = createMethod.Invoke(null, new object[] { settingsType }) as RecorderSettings;
                            if (settings != null)
                            {
                                settings.name = typeName;
                                MultiTimelineRecorderLogger.LogVerbose($"[RecorderClipUtility] Created {settingsType.Name} using RecordersInventory");
                                MultiTimelineRecorderLogger.Log($"[RecorderClipUtility] IMPORTANT: Actual created type is {settings.GetType().FullName}");
                                ...
                                {
                                    MultiTimelineRecorderLogger.LogError($"[RecorderClipUtility] ERROR: Requested FBX recorder but got Animation recorder! Rejecting.");
            ...
            MultiTimelineRecorderLogger.LogError($"[RecorderClipUtility] Unknown recorder type: {typeName}");
            return null;
        }
